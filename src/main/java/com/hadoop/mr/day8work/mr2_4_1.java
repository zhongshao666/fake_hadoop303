...
public class mr2_4_1 extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        System.exit(ToolRunner.run(new mr2_4_1(), args));
    }

    @Override
    public int run(String[] strings) throws Exception {
        Configuration conf = this.getConf();
        Path in = new Path("/data/mr2/data4");//输入路径
        Path out = new Path("day8work4_1");//输出路径

        Job job = Job.getInstance(conf, "word1");
        job.setJarByClass(this.getClass());

        job.setMapperClass(mr2_4Mapper.class);
        ...
        job.setReducerClass(mr2_4Reducer.class);
    ...
    static class mr2_4Mapper extends Mapper<LongWritable, Text, Text, IntWritable> {//map输出格式
    ...
    static class mr2_4Reducer extends Reducer<Text, IntWritable, Text, IntWritable> {//reduce输入输出格式
