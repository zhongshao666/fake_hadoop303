...
public class Wea2 extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        System.exit(ToolRunner.run(new Wea2(), args));
    }
    ...
    public int run(String[] strings) throws Exception {
        ...
        Path out = new Path("./day6work2");//输出路径

        Job job = Job.getInstance(conf, "Wea2");
        job.setJarByClass(this.getClass());

        job.setMapperClass(Wea2Mapper.class);
        job.setMapOutputKeyClass(Text.class);//map key2输出格式
        job.setMapOutputValueClass(Text.class);//map value2输出格式
        MultipleInputs.addInputPath(job,in1,TextInputFormat.class,Wea2Mapper.class);
        MultipleInputs.addInputPath(job,in2,TextInputFormat.class,Wea2Mapper.class);
        MultipleInputs.addInputPath(job,in3,TextInputFormat.class,Wea2Mapper.class);
        MultipleInputs.addInputPath(job,in4,TextInputFormat.class,Wea2Mapper.class);
        MultipleInputs.addInputPath(job,in5,TextInputFormat.class,Wea2Mapper.class);
        MultipleInputs.addInputPath(job,in6,TextInputFormat.class,Wea2Mapper.class);
        ...
        job.setReducerClass(Wea2Reducer.class);
    ...
    static class Wea2Mapper extends Mapper<LongWritable, Text, Text, Text> {//map输出格式
...
//                boolean matches = substring2.matches("[01459]");
                if (substring2.equals("0") || substring2.equals("1") || substring2.equals("4") || substring2.equals("5") || substring2.equals("9")) {
                    context.write(new Text(string.substring(0,19)), new Text(str1.substring(0, 4)));
                }
            }
        }
    }

    static class Wea2Reducer extends Reducer<Text, Text, Text, FloatWritable> {//reduce输入输出格式
